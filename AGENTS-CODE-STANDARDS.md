...
# When checkpoint (SinceTime) is provided, exit early when first older record found
# (all remaining records will also be older due to chronological ordering)
for ($i = $lines.Count - 1; $i -ge 0; $i--) {
    # Process line

    # Checkpoint optimization: break instead of continue when record is older than checkpoint
    if ($SinceTime -and $timestamp -le $SinceTime) {
        break  # Exit loop - all remaining records are older
    }
}
```
